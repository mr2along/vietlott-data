...
def main() -> None:
    ...
    print(f"NORMALIZED_BENCHMARK_ERROR={has_normalized_error}")
    ...
    if not has_complete_row_validation:
        raise SystemExit("Runner identity check failed: normalized 7-number validation missing")
    if not has_normalized_error:
        raise SystemExit("Runner identity check failed: normalized benchmark guard missing")
    if not has_benchmark_path:
        raise SystemExit("Runner identity check failed: benchmark dataset path missing")
